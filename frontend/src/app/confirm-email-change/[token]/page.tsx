        ...
        <div className="card p-8 text-center space-y-5">
          {status === 'loading' && (
            <>
              ...
              <p className="text-forge-muted">Confirming email change...</p>
            </>
          )}
          {status === 'success' && (
            <>
              <CheckCircle size={40} className="text-green-400 mx-auto" />
              <h2 className="font-display text-2xl text-white">Email Updated</h2>
              <p className="text-forge-muted">{message}</p>
              <Link href="/settings" className="btn-primary inline-block px-6 py-2">
                Go to Settings
              </Link>
            </>
          )}
